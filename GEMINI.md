...
- **【厳守】Pushの禁止:** **いかなる場合も、自律的に `git push` を実行してはいけません。** コミットが完了した段階で作業を止め、「コミットが完了しました。Pushしますか」とユーザーに確認してください。ユーザーがpushしてくださいと依頼されたらpush処理してもokです。
...
## 9. 状況説明
- 自動で色々やっている間、ユーザーが何をやっているのかわかるように、簡単で良いので説明しながら進めてください。

## 10. セッションログによる文脈の維持（中断・再開）
- ユーザーとの会話や作業状況を維持するため、プロジェクトルートにある `session_log.md` を活用してください。
- **セッション開始時（作業再開時）**: 必ず最初に `session_log.md` を読み込み、前回までの進捗、解決済みの課題、未解決の問題点、次やるべきタスクを把握してください。
- **セッション終了時（作業中断時）**: 作業に区切りがついた際やユーザーが離席する際は、必ず自律的に `session_log.md` を最新の状態に更新し、次に再開する自分自身へ引き継ぎを行ってください。
- セッションがクリアされても、きちんと再開できるようなるべく詳細に更新すること

## 11. 設計方針
- オブジェクト指向を用いた部品化をしっかりおこなうこと
- 可読性の高いコードを心がけること
- 再利用しやすいコードを心がけること
- テストしやすいコードを心がけること
- デバッグしやすい仕組みを作ること
- 高凝集・疎結合を心がけること
- クラス名、メソッド名、変数名は、役割がわかりやすい英語にすること
- コメントは適切な箇所に適切に記述すること。ただし、変数名やメソッド名が役割が分かりやすい場合は冗長なコメントは不要。
- クラス名は全体のバランスを考慮して命名すること(必要に応じてリファクタリングして命名を変更可)
- UIとロジックは疎結合にすること


## 12. 実装の方針(MZ1500エミュレータ)
- MZ1500のC# エミュレーターについては、`CommonSourceProjectSrc` フォルダ以下のソースコードを参考にした実装を行うこと
- CommonSourceProjectSrcを調べて得られた MZ1500のエミュレータのファイル構成については、以降対象ファイルをしらべなくてもいいように、記録しておくこと mz1500_emulator_file_structure_from_cpp.mdに追記すること。
- CommonSourceProjectSrcから得られたエミュレータの仕様(=MZ1500の仕様)は、今後のためにも、mz1500_emulator_spec_from_cpp.mdファイルにまとめておくこと。

## 13. UIのデザイン
- VS Code のような洗練された デザインとすること

## 14. デバッグ
- デバッグが出来る環境なので、調査をするために、必要であればユーザーへ指示すること(スタックトレースなどの情報は得られる)