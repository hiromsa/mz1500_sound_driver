...
public class MZ1500SoundDriverGenerator
{
    ...
    public byte[] Build(byte[]? pcgData = null)
    {
        ...
        {
            assembler.LD(assembler.IX, assembler.LabelRef(new AsmLabel(ch.Name + "_DataBlock")));
            if (ch.IOPort == 0xE0) {
                assembler.CALL(assembler.LabelRef(new AsmLabel("UpdateBeepChannel")));
            } else {
                assembler.CALL(assembler.LabelRef(new AsmLabel("UpdatePSGChannel")));
            }
        }
        ...
        foreach (var ch in ChannelList)
        {
            assembler.Label(new AsmLabel(ch.Name + "_DataBlock"));
            assembler.DW(assembler.LabelRef(new AsmLabel(ch.Name + "_SongData")));
            assembler.DW(assembler.LabelRef(new AsmLabel(ch.Name + "_SongDataEnd")));
            ...
            assembler.DB(0);
            assembler.DB(0);
            assembler.DW(assembler.LabelRef(new AsmLabel("global_env_data_empty")));
            assembler.DB(0);
            assembler.DB(0);
            assembler.DW(assembler.LabelRef(new AsmLabel("global_penv_data_empty")));
            assembler.DB(0);
        ...
        foreach (var ch in ChannelList)
        {
            assembler.Label(new AsmLabel(ch.Name + "_SongData"));
            assembler.DB(ch.SequenceData);
            assembler.Label(new AsmLabel(ch.Name + "_SongDataEnd"));
            assembler.DB(0xFF);
        }
...
private void AppendSharedPlayRoutine(Z80Assembler asm, bool isBeep)
    {
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel" : "UpdatePSGChannel"));
        ...
        asm.OR(asm.A);
        asm.JP(asm.NZ, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_DecDurLower" : "UpdatePSGChannel_DecDurLower")));
        // lower is 0, check upper
        asm.INC(asm.HL);
        asm.LD(asm.A, asm.HLref);
        asm.OR(asm.A);
        asm.DEC(asm.HL); // restore HL
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne"))); // both 0 -> next command
        ...
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_DecDurLower" : "UpdatePSGChannel_DecDurLower"));
        asm.DEC(asm.HLref);
        ...
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputSoundByStatus" : "UpdatePSGChannel_OutputSoundByStatus")));

        
        // 3. 隹ｺ・｡邵ｺ・ｮ郢ｧ・ｳ郢晄ｧｭﾎｦ郢晏ｳｨ・帝坡・ｭ郢ｧﾂ陷・ｽｦ騾・・
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne"));
        ...
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_EndSong" : "UpdatePSGChannel_EndSong")));

        // 0x60 (Rest)
        asm.CP(asm.Value((byte)0x60));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadKyufuData" : "UpdatePSGChannel_ReadKyufuData")));

        // 0xA0 (Set Voice / Env)
        asm.CP(asm.Value((byte)0xA0));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadEnvData" : "UpdatePSGChannel_ReadEnvData")));

        // 0xA1 (Set Volume)
        asm.CP(asm.Value((byte)0xA1));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadVolumeData" : "UpdatePSGChannel_ReadVolumeData")));

        // 0xA2 (Set PEnv)
        asm.CP(asm.Value((byte)0xA2));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadPEnvData" : "UpdatePSGChannel_ReadPEnvData")));

        // 0x90 (Long Length)
        asm.CP(asm.Value((byte)0x90));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadLongLen" : "UpdatePSGChannel_ReadLongLen")));

        // 0x08 (Loop Marker)
        asm.CP(asm.Value((byte)0x08));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadLoopMarker" : "UpdatePSGChannel_ReadLoopMarker")));

        // Noise (0x06) -> wait, we kept Noise as 0x06
        asm.CP(asm.Value((byte)0xA6));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadNoise" : "UpdatePSGChannel_ReadNoise")));

        // Sync Noise (0x07)
        asm.CP(asm.Value((byte)0xA7));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSyncNoise" : "UpdatePSGChannel_ReadSyncNoise")));

        // If A < 0x60, it's Note ON (0x00 - 0x5F)
        asm.CP(asm.Value((byte)0x60));
        asm.JP(asm.C, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadToneData" : "UpdatePSGChannel_ReadToneData")));
        
        // If A >= 0x80 AND A <= 0x8F, it's Short Length
        asm.SUB(asm.Value((byte)0x80));
        asm.CP(asm.Value((byte)0x10));
        asm.JP(asm.C, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadShortLen" : "UpdatePSGChannel_ReadShortLen")));

        // Unknown -> Ignore and read next
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne")));

        // -- Read Long Length --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadLongLen" : "UpdatePSGChannel_ReadLongLen"));
        asm.LD(asm.A, asm.DEref);
        asm.INC(asm.DE);
        asm.LD(asm.C, asm.A);
        asm.LD(asm.A, asm.DEref);
        ...
        // -- Read Short Length --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadShortLen" : "UpdatePSGChannel_ReadShortLen"));
        ...
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne")));
// -- Read Loop Marker
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadLoopMarker" : "UpdatePSGChannel_ReadLoopMarker"));
        ...
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadEnvData" : "UpdatePSGChannel_ReadEnvData"));
        ...
        asm.LD(asm.A, 0xFF);
        asm.CP(asm.B);
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_env_off")));
        ...
        asm.LD(asm.HL, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_DataEnvTableBase" : "UpdatePSGChannel_DataEnvTableBase")));
        asm.LD(asm.A, asm.B); 
        ...
        // Env Off
        asm.Label(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_env_off"));
        asm.LD(asm.IXref(IxStatEnvActive), asm.Value((byte)0x00));
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne")));

        // -- Read Pitch Envelope Command --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadPEnvData" : "UpdatePSGChannel_ReadPEnvData"));
        ...
        asm.LD(asm.A, 0xFF);
        asm.CP(asm.B);
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_penv_off")));
        ...
        asm.LD(asm.HL, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_DataPEnvTableBase" : "UpdatePSGChannel_DataPEnvTableBase")));
        asm.LD(asm.A, asm.B); 
        asm.ADD(asm.A, asm.A); // A = EnvId * 2
        // Calculate HL + A
        asm.LD(asm.C, asm.A);
        ...
        // PEnv Off
        asm.Label(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_penv_off"));
        asm.LD(asm.IXref(IxStatPEnvActive), asm.Value((byte)0x00));
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne")));

        
        // -- Read Tone -- 
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadToneData" : "UpdatePSGChannel_ReadToneData"));
        ...
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel" : "UpdatePSGChannel")));

        // -- Read Rest --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadKyufuData" : "UpdatePSGChannel_ReadKyufuData"));
...
// -- Read Noise --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadNoise" : "UpdatePSGChannel_ReadNoise"));
        ...
        // -- Read Sync Noise --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSyncNoise" : "UpdatePSGChannel_ReadSyncNoise"));
        asm.LD(asm.IXref(IxStatEnvPosOffset), asm.Value((byte)0x00));
        asm.LD(asm.IXref(IxStatPEnvPosOffset), asm.Value((byte)0x00));
        asm.LD(asm.IXref(IxStatNoteOn), asm.Value((byte)0x01));
...
// -- Read Volume --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadVolumeData" : "UpdatePSGChannel_ReadVolumeData"));
        ...
        // -- End Song (Looping) --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_EndSong" : "UpdatePSGChannel_EndSong"));
        asm.LD(asm.L, asm.IXref(IxStatLoopPosition));
        asm.LD(asm.H, asm.IXref((sbyte)(IxStatLoopPosition + 1)));
        ...
        asm.JP(asm.NZ, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_ReadSongDataOne" : "UpdatePSGChannel_ReadSongDataOne")));
        ...
        // -- Output By Status --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputSoundByStatus" : "UpdatePSGChannel_OutputSoundByStatus"));
        // Check Note On
        asm.LD(asm.A, asm.IXref(IxStatNoteOn));
        asm.OR(asm.A);
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputEnd" : "UpdatePSGChannel_OutputEnd"))); // Note Off -> Do nothing

        // Check Env Active
        asm.LD(asm.A, asm.IXref(IxStatEnvActive));
        asm.OR(asm.A);
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputPenvCheck" : "UpdatePSGChannel_OutputPenvCheck"))); // Env Off -> skip to PEnv
        ...
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvLoopEnd" : "UpdatePSGChannel_EnvLoopEnd")));

        // it might be End marker (0xFF)
        asm.CP(asm.Value((byte)0xFF));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvEnd" : "UpdatePSGChannel_EnvEnd")));
        ...
        if (isBeep) {
            ...
            asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvVolMute" : "UpdatePSGChannel_EnvVolMute")));
            asm.LD(asm.A, (byte)1);
            asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvVolApply" : "UpdatePSGChannel_EnvVolApply")));
            
            asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvVolMute" : "UpdatePSGChannel_EnvVolMute"));
            asm.LD(asm.A, (byte)0);
            
            asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvVolApply" : "UpdatePSGChannel_EnvVolApply"));
            asm.LD(asm.HL, (ushort)0xE008);
            asm.LD(asm.HLref, asm.A);
        } else {
        ...
        // Increment Offset
        asm.LD(asm.HL, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_StatEnvPosOffset" : "UpdatePSGChannel_StatEnvPosOffset")));
        asm.INC(asm.HLref);

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputPenvCheck" : "UpdatePSGChannel_OutputPenvCheck"));
        // Check PEnv Active
        asm.LD(asm.A, asm.IXref(IxStatPEnvActive));
        asm.OR(asm.A);
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputEnd" : "UpdatePSGChannel_OutputEnd"))); // PEnv Off -> End
        ...
        // is it Loop Endpoint? (0xFE)
        asm.CP(asm.Value((byte)0xFE));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_PenvLoopEnd" : "UpdatePSGChannel_PenvLoopEnd")));

        // it might be End marker (0xFF)
        asm.CP(asm.Value((byte)0xFF));
        asm.JP(asm.Z, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_PenvEnd" : "UpdatePSGChannel_PenvEnd")));
        ...
        // Increment Offset
        asm.LD(asm.HL, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_StatPEnvPosOffset" : "UpdatePSGChannel_StatPEnvPosOffset")));
        asm.INC(asm.HLref);

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputEnd" : "UpdatePSGChannel_OutputEnd"));
        asm.RET();

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvLoopEnd" : "UpdatePSGChannel_EnvLoopEnd"));
        ...
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_EnvEnd" : "UpdatePSGChannel_EnvEnd"));
        ...
        // PEnv loop handlers
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_PenvLoopEnd" : "UpdatePSGChannel_PenvLoopEnd"));
        asm.INC(asm.HL);
        asm.LD(asm.A, asm.HLref);
        asm.LD(asm.IXref(IxStatPEnvPosOffset), asm.A);
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputPenvCheck" : "UpdatePSGChannel_OutputPenvCheck")));
        
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_PenvEnd" : "UpdatePSGChannel_PenvEnd"));
        asm.LD(asm.HL, asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_StatPEnvPosOffset" : "UpdatePSGChannel_StatPEnvPosOffset")));
        asm.DEC(asm.HLref);
        asm.JP(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_OutputPenvCheck" : "UpdatePSGChannel_OutputPenvCheck")));
        ...
        // -- Stat Variables --
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatSongDataPosition" : "UpdatePSGChannel_StatSongDataPosition"));
        asm.DB(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_DataSong" : "UpdatePSGChannel_DataSong"))); // Initialize with Data Start Address
        
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatLoopPosition" : "UpdatePSGChannel_StatLoopPosition"));
        asm.DB(asm.LabelRef(new AsmLabel(isBeep ? "UpdateBeepChannel_DataSongEnd" : "UpdatePSGChannel_DataSongEnd"))); // Initialize loop point to End Address (No loop by default)

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatLengthRemain" : "UpdatePSGChannel_StatLengthRemain"));
        asm.DB(new byte[] { 0, 0 });
        
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatLastLength" : "UpdatePSGChannel_StatLastLength"));
        asm.DB(new byte[] { 0, 0 });
        
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatGateRemain" : "UpdatePSGChannel_StatGateRemain"));
        asm.DB(new byte[] { 0, 0 });

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatNoteOn" : "UpdatePSGChannel_StatNoteOn"));
        asm.DB(0);

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatHwVolume" : "UpdatePSGChannel_StatHwVolume"));
        asm.DB(0); // Holds the raw SN76489 volume byte

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatEnvActive" : "UpdatePSGChannel_StatEnvActive"));
        asm.DB(0); 

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatEnvDataPtr" : "UpdatePSGChannel_StatEnvDataPtr"));
        asm.DB(new byte[] { 0, 0 });

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatEnvPosOffset" : "UpdatePSGChannel_StatEnvPosOffset"));
        asm.DB(0);

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatPEnvActive" : "UpdatePSGChannel_StatPEnvActive"));
        asm.DB(0); 

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatPEnvDataPtr" : "UpdatePSGChannel_StatPEnvDataPtr"));
        asm.DB(new byte[] { 0, 0 });

        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_StatPEnvPosOffset" : "UpdatePSGChannel_StatPEnvPosOffset"));
        asm.DB(0);
        ...
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_DataEnvTableBase" : "UpdatePSGChannel_DataEnvTableBase"));
        ...
        for (int i = 0; i <= maxEnvId; i++)
        {
            ...
            {
                asm.DW(asm.LabelRef(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_env_data_{i}")));
            }
            else
            {
                // Dummy/empty
                asm.DW(asm.LabelRef(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_env_data_empty")));
            }
        }

        // Dummy empty data
        asm.Label(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_env_data_empty"));
        asm.DB(0xFF);

        // Env Array Definitions
        foreach (var kvp in VolumeEnvelopes)
        {
            asm.Label(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_env_data_{kvp.Key}"));
        ...
        asm.Label(new AsmLabel(isBeep ? "UpdateBeepChannel_DataPEnvTableBase" : "UpdatePSGChannel_DataPEnvTableBase"));
        ...
        for (int i = 0; i <= maxPEnvId; i++)
        {
            ...
            {
                asm.DW(asm.LabelRef(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_penv_data_{i}")));
            }
            else
            {
                asm.DW(asm.LabelRef(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_penv_data_empty")));
            }
        }

        asm.Label(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_penv_data_empty"));
        asm.DB(0xFF);
        asm.DB(0xFF); // Align 2 bytes

        foreach (var penv in HwPitchEnvelopes)
        {
            asm.Label(new AsmLabel($"{new AsmLabel(isBeep ? "UpdateBeepChannel_Prefix" : "UpdatePSGChannel_Prefix")}_penv_data_{penv.Id}"));
    ...
    {
        asm.Label(new AsmLabel("global_env_table"));
        int maxEnvId = -1;
        ...
        for (int i = 0; i <= maxEnvId; i++) {
            if (VolumeEnvelopes.ContainsKey(i))
                asm.DW(asm.LabelRef(new AsmLabel($"global_env_data_{i}")));
            else
                asm.DW(asm.LabelRef(new AsmLabel("global_env_data_empty")));
        }
        
        asm.Label(new AsmLabel("global_env_data_empty"));
        asm.DB(0xFF);
        
        foreach (var kvp in VolumeEnvelopes) {
            asm.Label(new AsmLabel($"global_env_data_{kvp.Key}"));
            var envData = kvp.Value;
        ...
        asm.Label(new AsmLabel("global_penv_table"));
        int maxPEnvId = -1;
        if (HwPitchEnvelopes.Count > 0) maxPEnvId = HwPitchEnvelopes.Count - 1;
        for (int i = 0; i <= maxPEnvId; i++) {
            if (i < HwPitchEnvelopes.Count)
                asm.DW(asm.LabelRef(new AsmLabel($"global_penv_data_{i}")));
            else
                asm.DW(asm.LabelRef(new AsmLabel("global_penv_data_empty")));
        }
        
        asm.Label(new AsmLabel("global_penv_data_empty"));
        asm.DB(0xFF); asm.DB(0xFF);
        
        foreach (var penv in HwPitchEnvelopes) {
            asm.Label(new AsmLabel($"global_penv_data_{penv.Id}"));
            foreach (ushort hwVal in penv.AbsoluteRegisters) {
                asm.DB((byte)(hwVal & 0xFF));
                asm.DB((byte)((hwVal >> 8) & 0xFF));
            }
